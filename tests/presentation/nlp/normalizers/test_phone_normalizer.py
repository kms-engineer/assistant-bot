...
class TestPhoneNormalizer:
    ...
    def test_normalize_valid_phone_with_library(self, input_phone, region, expected_e164, expected_national):
        """Tests normalization of valid phone numbers when 'phonenumbers' is available."""
        with patch('src.presentation.nlp.normalizers.phone_normalizer.PHONENUMBERS_AVAILABLE', True):
    ...
    def test_normalize_without_library(self):
        ...
        with patch('src.presentation.nlp.normalizers.phone_normalizer.PHONENUMBERS_AVAILABLE', False):
