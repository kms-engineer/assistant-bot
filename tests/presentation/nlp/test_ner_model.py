...
class TestNERModel:
    ...
    @patch('src.presentation.nlp.ner_model.AutoModelForTokenClassification.from_pretrained')
    @patch('src.presentation.nlp.base_model.AutoTokenizer.from_pretrained')
    @patch('os.path.exists', return_value=True)
    @patch('builtins.open', new_callable=MagicMock)
    @patch('json.load')
    def test_extract_entities_returns_tuple(self, mock_json_load, mock_open, mock_exists,
                                           mock_tokenizer_loader, mock_model_loader,
                                           mock_pipeline_loader, mock_ner_components):
    ...
    @patch('builtins.open', new_callable=MagicMock)
    @patch('json.load')
    def test_extract_entities_has_expected_keys(self, mock_json_load, mock_open, mock_exists,
                                                mock_tokenizer_loader, mock_model_loader,
                                                mock_pipeline_loader, mock_ner_components):
        ...
        expected_keys = ["name", "phone", "email", "address", "birthday", "tag", "note_text", "id", "days"]
    ...
    @patch('builtins.open', new_callable=MagicMock)
    @patch('json.load')
    def test_extract_entities_with_intent_filtering(self, mock_json_load, mock_open, mock_exists,
                                                    mock_tokenizer_loader, mock_model_loader,
        ...
        mock_pipeline.return_value = [
            ...
            {'entity_group': 'EMAIL', 'score': 0.90, 'start': 30, 'end': 45}  # Should be filtered out
        ]
        ...
        entities, _ = ner_model.extract_entities("Add John 1234567890 test@email.com", intent="add_contact")
    ...
    @patch('builtins.open', new_callable=MagicMock)
    @patch('json.load')
    def test_extract_entities_empty_text(self, mock_json_load, mock_open, mock_exists,
                                        mock_tokenizer_loader, mock_model_loader,
                                        mock_pipeline_loader):
    ...
    def test_parse_ner_results_possessive_name(self):
        ...
        ner_results = [
            {'entity_group': 'NAME', 'score': 0.95, 'start': 0, 'end': 7}
        ]
    ...
    def test_parse_ner_results_with_filtering(self):
        ...
        entities, confidences = NERModel._parse_ner_results(ner_results, text, allowed_entities)
...
class TestNERModelEdgeCases:
    ...
    @patch('builtins.open', new_callable=MagicMock)
    @patch('json.load')
    def test_extract_entities_special_characters(self, mock_json_load, mock_open, mock_exists,
                                                 mock_tokenizer_loader, mock_model_loader,
                                                 mock_pipeline_loader):
    ...
    @patch('builtins.open', new_callable=MagicMock)
    @patch('json.load')
    def test_extract_entities_very_long_text(self, mock_json_load, mock_open, mock_exists,
                                             mock_tokenizer_loader, mock_model_loader,
                                             mock_pipeline_loader):
