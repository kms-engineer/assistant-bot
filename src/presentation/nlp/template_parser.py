...
class TemplateParser:
    ...
    def _extract_basic_entities(user_text: str, existing_entities: Dict) -> Dict:
        ...
        if 'birthday' not in entities:
            match = re.search(RegexPatterns.BIRTHDAY_PATTERN_COMBINED, user_text, re.IGNORECASE)
            if match:
        ...
        if 'name' not in entities:
            ...
            if matches:
                ...
                for match in matches:
                    words = match.split()
                    if not any(word in EntityConfig.NAME_EXCLUDED_WORDS for word in words):
                        entities['name'] = match
                        break
        ...
        if 'address' not in entities:
            # Street address
            match = re.search(RegexPatterns.TEMPLATE_ADDRESS_STREET_FULL, user_text, re.IGNORECASE)
            if match:
                entities['address'] = match.group(0)
            else:
                # "from City, Country" format
                match = re.search(RegexPatterns.TEMPLATE_ADDRESS_FROM_PATTERN, user_text, re.IGNORECASE)
                if match:
